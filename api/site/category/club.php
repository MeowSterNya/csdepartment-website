...
require_once "../object/club_obj.php";
...
$club->id = ( isset( $_GET[ "id" ] ) ? $_GET[ "id" ] : 0 );
$bGetAll = empty( $club->id );

// retrieve the requested data from the database via the Service API, in JSON format
$result_data_json = ( $bGetAll ? $club->read() : $club->readOne() );
// Ö and, strip off any UTF-8 BOM prefix
...
if ( $bGetAll ) {
    ...
    $result_data = "<h1>Club Details</h1>";
...
echo "<!DOCTYPE html>" . 
     "<html>" . 
     "<head>" . 
     "<title>" . ( $bGetAll ? "Club List" : "Club Details" ) . "</title>" . 
     '<meta charset="utf-8">' . 
     "</head>" . 
     "<body>" . 
     $result_data . 
