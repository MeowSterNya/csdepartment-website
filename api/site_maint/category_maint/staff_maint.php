...
$form = file_get_contents( "staff_maint_form.html" );
...
if ( ! empty( $_POST[ "submit" ] ) ) {
    ...
    require_once "../../object_full/staff_obj_full.php";
    
    // initialisation
    $db = new Database();
    $db_conn = $db->getConnection();
    $staff_member = new StaffMember( $db_conn );
    
    // retrieve the submitted form data
    $operation = $_POST[ "operation" ];
    if ( ( $operation == "update" ) || ( $operation == "delete" ) ) {
        if ( empty( $_POST[ "id" ] ) ) {
            die( "Sorry, unable to " . $operation . " Staff member. ID is missing." );
        }
        $staff_member->id = $_POST[ "id" ];
    }
    $staff_member->first_name = ( ! empty( $_POST[ "first_name" ] ) ? $_POST[ "first_name" ] : "" );
    $staff_member->last_name = ( ! empty( $_POST[ "last_name" ] ) ? $_POST[ "last_name" ] : "" );
    $staff_member->age = ( ! empty( $_POST[ "age" ] ) ? $_POST[ "age" ] : "" );
    $staff_member->category_id = ( ! empty( $_POST[ "category_id" ] ) ? $_POST[ "category_id" ] : "" );
    ...
    if ( ( $operation == "create" ) || ( $operation == "update" ) ) {
        ...
        foreach ( $fileUploadFields as $thisField ) {
            $thisUploadDir = '../../uploads/staff/' . $thisField . ( ( $thisField == 'photo' ) ? 's' : '' ) . '/';
            $thisFieldPathnameVarName = $thisField . '_pathname';
            $staff_member->$thisFieldPathnameVarName = "";
                ...
                switch ( $_FILES[ $thisField ][ 'error' ] ) {
                    case UPLOAD_ERR_OK:
                        ...
                        if ( move_uploaded_file( $_FILES[ $thisField ][ 'tmp_name' ], 
                            ...
                            $staff_member->$thisFieldPathnameVarName = $$thisFieldPathnameVarName;
    ...
    if ( $operation == "create" ) {
        ...
        if ( empty( $staff_member->first_name ) || empty( $staff_member->last_name ) || 
             empty( $staff_member->age ) || empty( $staff_member->category_id ) ) {
            die( "Sorry, unable to create Staff member. Data is incomplete." );
        }
    }
    ...
    switch ( $operation ) {
        case "create":
            $staff_member->created = date( 'Y-m-d H:i:s' );
            $action_result = $staff_member->create();
            break;
            
        case "update":
            $action_result = $staff_member->update();
            break;
            
        case "delete":
            $action_result = $staff_member->delete();
            break;
    ...
    if ( ( is_int( $action_result ) ) && ( $action_result == $k_MyErr_UnknownOp ) ) {
        ...
        // tell the user
        echo "<h3>Sorry, staff member already exists!</h3>";
        ...
        // tell the user
        echo "<h3>Staff member was " . ( $operation . "d" ) . ".</h3>";
        ...
        // tell the user
        echo "<h3>Sorry, unable to " . $operation . " Staff member!</h3>";
    }
}
?>
